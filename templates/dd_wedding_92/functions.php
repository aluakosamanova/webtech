...
if (!defined('_DESIGNER_FUNCTIONS')) {
    ...
    function processingShortcodes($content, $isModuleContent = false) {
        if ($GLOBALS['theme_settings']['is_preview']) {
            ...
            if ($isModuleContent) {
                $plgScFile = JPATH_SITE . '/plugins/content/themlercontent/themlercontent.php';
                $sh = JPATH_SITE . '/plugins/content/themlercontent/lib/Shortcodes.php';
                if (file_exists($plgScFile) && file_exists($sh)) {
                    require_once $sh;
                    $content = DesignerShortcodes::process($content);
                }
            }
